...
void CreateVoteMenu(int iSlot, int iTarget, int iType)
{
	...
	g_bActiveVote = true;
	g_pUtils->CreateTimer(g_iResultTimer, [iTarget, iType]() {
		if(g_bActiveVote == false) return -1.0f;
		g_bActiveVote = false;
		int iCount = 0;
	...
	for (int i = 0; i < 64; i++)
	{
		...
		char szBuffer[64];
		static const char* szPhrase[] = {"VoteBKM_Title_Ban", "VoteBKM_Title_Mute", "VoteBKM_Title_Gag", "VoteBKM_Title_Kick"};
		g_SMAPI->Format(szBuffer, sizeof(szBuffer), g_pAdminApi->GetTranslation(szPhrase[iType]), g_pPlayersApi->GetPlayerName(iTarget));
		...
		g_pMenusApi->SetCallback(hMenu, [iTarget, iType](const char* szBack, const char* szFront, int iItem, int iSlot) {
			if(iItem < 7)
			{
				if(g_bActiveVote == false) {
					g_pUtils->PrintToChat(iSlot, g_pAdminApi->GetTranslation("VoteBKM_AlreadyEnded"));
					return;
				}
				...
				if(iVote == 0)
				{
					...
					static const char* szPhrase[] = {"VoteBKM_Action_Ban", "VoteBKM_Action_Mute", "VoteBKM_Action_Gag", "VoteBKM_Action_Kick"};
					g_pUtils->PrintToChatAll(g_pAdminApi->GetTranslation(szPhrase[iType]), g_pPlayersApi->GetPlayerName(iSlot), g_pPlayersApi->GetPlayerName(iTarget), iCount, iNeeded);
					if(iCount >= iNeeded) CheckPunishment(iTarget);
				}
	...
	g_pMenusApi->SetCallback(hMenu, [iType](const char* szBack, const char* szFront, int iItem, int iSlot) {
		if(iItem < 7)
		{
			...
			if(!g_iTypeVote)
			{
				...
				static const char* szPhrase[] = {"VoteBKM_Created_Ban", "VoteBKM_Created_Mute", "VoteBKM_Created_Gag", "VoteBKM_Created_Kick"};
				static const char* szPhraseAdmin[] = {"VoteBKM_Created_Ban_Admin", "VoteBKM_Created_Mute_Admin", "VoteBKM_Created_Gag_Admin", "VoteBKM_Created_Kick_Admin"};

				if(g_pAdminApi->GetMessageType() == 1)
					g_pUtils->PrintToChatAll(g_pAdminApi->GetTranslation(szPhrase[iType]), g_pPlayersApi->GetPlayerName(iTarget));
				else
					g_pUtils->PrintToChatAll(g_pAdminApi->GetTranslation(szPhraseAdmin[iType]), g_pPlayersApi->GetPlayerName(iSlot), g_pPlayersApi->GetPlayerName(iTarget));
...
void OnClientAutorized(int iSlot, uint64 iSteamID64)
{
	for (int i = 0; i < 64; i++)
	{
		for (int j = 0; j < 4; j++)
		{
			if(g_iTarget[i][j] == iSlot) g_iTarget[i][j] = -1;
		}
	}
}
...
void VoteBKM::AllPluginsLoaded()
{
	...
	g_pPlayersApi->HookOnClientAuthorized(g_PLID, OnClientAutorized);
	LoadConfig();
}
